...
def get_save(cam,wl,device,barcode,delay=None):
 if delay==None: delay=5
 wl_to_ser(wl,device,status='on')
 tm.sleep(delay)
 image,timestamps = ct.grab_images(cam)
 ct.save_avi(image,prefix=str(wl),barcode=barcode)
...
def set_expos_time(cam,time):
 ...
 try:
  cam.ExposureAuto.SetValue(ps.ExposureAuto_Off) # turn off auto exposure
  time = min(cam.ExposureTime.GetMax(), time) #checks to be sure 'time' isn't longer than max possible
...
def print_cam_info(cam):
 ...
 try:
  ...
  if ps.IsAvailable(node_device_info) and ps.IsReadable(node_device_infor):
   features = node_device_info.GetFeatures()
   for feature in features:
    node_feat = ps.CValuePtr(feature)
    print('%s: %s' % (node_feat.GetName(),node_feat.ToString()))
 except ps.SpinnakerException as ex:
  print('Error: %s' % ex)
  return False
...
def set_framerate(cam,frame_rate=None):
 ...
 if frame_rate==None: frame_rate=158.0
 try: 
...
def grab_images(cam, length=None, n_frames=None):
 ...
 seconds"""
 if length==None and n_frames==None: n_frames=1
 elif not length==None and n_frames==None:
  frame_rate= get_framerate(cam)
  exposure_time=cam.ExposureTime()
  if 1000**2/exposure_time<frame_rate: frame_rate=1000**2/exposure_time
...
def save_avi(images,frame_rate=None,barcode=None,prefix=None,path=None,is_color=None):
 ...
 if frame_rate==None: frame_rate=160.0
 if barcode==None: barcode='00000'
 if prefix==None: prefix='videofile'
 if path==None: path='./'
 if is_color==None: is_color=False
